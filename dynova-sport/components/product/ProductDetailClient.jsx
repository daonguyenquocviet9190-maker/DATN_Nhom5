          ...
          <div className="order-2 flex gap-3 overflow-x-auto pb-1 lg:order-1 lg:flex-col lg:overflow-visible lg:pb-0">
            {gallery?.map((image, index) => (
              <button
                ...
                className={`h-20 w-20 shrink-0 overflow-hidden rounded-2xl border-2 bg-slate-100 transition hover:-translate-y-0.5 lg:h-[86px] lg:w-[86px] ${
                  mainImage === image
            ...
            <div className="mt-6 rounded-[24px] bg-gradient-to-r from-orange-50 to-slate-50 p-5">
              ...
              {originalPrice && Number(originalPrice) > Number(displayPrice) && (
                <p className="mt-2 inline-flex rounded-full bg-orange-500 px-3 py-1 text-xs font-black text-white">
                  Tiết kiệm {formatCurrency(Number(originalPrice) - Number(displayPrice))}
                </p>
              )}
            </div>

            {/* Variants configuration (Colors & Sizes) */}
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {colorOptions.map((color) => {
                      const active = String(selectedColorId) === String(color.id);
                      return (
                        <button
                          key={color.id}
                          type="button"
                          onClick={() => handleColorChange(color.id)}
                          className={`flex min-h-12 items-center gap-2 rounded-2xl border px-4 py-2.5 text-sm font-black transition ${
                            active
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {sizeOptions.map((size) => {
                      const active = String(selectedSizeId) === String(size.id);
                      ...
                      return (
                        <button
                          ...
                          className={`flex h-12 min-w-12 items-center justify-center rounded-2xl border px-4 text-sm font-black transition ${
                            active
        ...
        <div className="mb-7 flex gap-3 overflow-x-auto border-b border-slate-200">
          {[
            ...
            <button
              ...
              className={`shrink-0 pb-4 text-sm font-black uppercase tracking-wider transition ${
                activeTab === tab.id
        ...
        {activeTab === "specs" && (
          <div className="grid gap-4 md:grid-cols-2">
            {[
              ...
              ["Thương hiệu", brandName || "Đang cập nhật"],
              ["Danh mục", categoryName || "Đang cập nhật"],
              ["Giá hiện tại", formatCurrency(displayPrice)],
              [
                "Giá gốc",
                originalPrice ? formatCurrency(originalPrice) : "Không áp dụng",
              ],
              [
                "Màu đang chọn",
                selectedColor?.name || "Không có phân loại màu",
              ],
              [
                "Size đang chọn",
                selectedSize?.name || "Không có phân loại size",
              ],
