...
function getPaymentStatusLabel(order) {
  ...
  return ["bank", "bank_transfer", "vietqr"].includes(method)
    ? "Chờ thanh toán"
    : "Chưa thanh toán";
}
...
function getStatusMeta(status = "") {
  ...
  const map = {
    ...
    waiting_bank_transfer: {
      ...
      icon: Clock3,
      className: "bg-yellow-500/10 text-yellow-300 ring-yellow-400/20",
    },
    confirmed: {
      label: "Đã xác nhận",
      icon: PackageCheck,
    ...
    shipping: {
      label: "Đang giao",
      icon: Truck,
      className: "bg-indigo-500/10 text-indigo-300 ring-indigo-400/20",
    },
    completed: {
      label: "Hoàn thành",
      icon: CheckCircle2,
...
function getCreatedAt(order) {
  ...
  try {
    ...
    // Chuyển đổi và định dạng chính xác theo Múi giờ Việt Nam (GMT+7)
    ...
    return (
      <div className="flex flex-col">
        <span className="font-bold text-slate-300">{timeStr}</span>
                  ...
                  return (
                    <tr key={order.id} className="hover:bg-white/[0.04]">
                      ...
                      <td className="px-5 py-4 text-sm font-bold text-slate-300">
                        {getOrderItemsCount(order)} sản phẩm
                      </td>
                      ...
                      <td className="px-5 py-4">
                        <span
                          className={`inline-flex w-fit items-center gap-2 rounded-full px-3 py-1.5 text-[11px] font-black uppercase ring-1 ${meta.className}`}
                        >
                          <StatusIcon size={13} />
                          {meta.label}
                        </span>
