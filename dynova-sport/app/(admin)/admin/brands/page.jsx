...
import { BRAND_FALLBACK, getBrandImage } from "@/utils/imageUrl";
import { createAdminBrand, deleteAdminBrand, extractItems, getAdminBrands, updateAdminBrand } from "@/services/admin.service";
...
export default function AdminBrandsPage() {
  ...
  const loadData = async () => {
    try {
      setLoading(true);
      setError("");
      const response = await getAdminBrands({ per_page: 200 });
      setItems(extractItems(response, ["brands"]));
    } catch (err) {
  ...
  const submit = async (event) => {
    ...
    try {
      ...
      if (editing?.id) await updateAdminBrand(editing.id, payload);
      else await createAdminBrand(payload);
      setOpen(false);
      showNotice(editing ? "Đã cập nhật thương hiệu." : "Đã thêm thương hiệu.");
      await loadData();
    } catch (err) {
      setError(err?.message || "Không thể lưu thương hiệu. Kiểm tra API admin/brands.");
    } finally { setSaving(false); }
  };

  const remove = async (item) => {
    if (!confirm(`Xóa thương hiệu "${item.name}"?`)) return;
    try { await deleteAdminBrand(item.id); showNotice("Đã xóa danh mục."); await loadData(); }
  ...
  return (
    <div className="space-y-6">
      ...
      <section className="rounded-[32px] border border-white/10 bg-white/[0.06] p-5 backdrop-blur-xl">
        {loading ? <div className="grid h-64 place-items-center"><Loader2 className="animate-spin text-orange-300" size={34}/></div> : filtered.length === 0 ? <div className="grid h-64 place-items-center text-center"><div><Tags className="mx-auto text-orange-300" size={40}/><p className="mt-3 font-black text-white">Chưa có thương hiệu</p></div></div> : <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
          {filtered.map((item)=><div key={item.id} className="rounded-[26px] border border-white/10 bg-white/[0.04] p-4">
            <div className="flex gap-4"><img src={getBrandImage(item)} onError={(e)=>{e.currentTarget.src=BRAND_FALLBACK}} alt={item.name} className="h-20 w-20 rounded-2xl object-cover"/><div className="min-w-0 flex-1"><p className="truncate text-base font-black text-white">{item.name}</p><p className="mt-1 line-clamp-2 text-sm text-slate-500">{item.description || item.country || "Chưa có mô tả"}</p><span className={(item.is_active === false || item.is_active === 0 ? "bg-slate-500/10 text-slate-400" : "bg-emerald-500/10 text-emerald-300") + " mt-3 inline-flex rounded-full px-3 py-1 text-xs font-black"}>{item.is_active === false || item.is_active === 0 ? "Ẩn" : "Hiển thị"}</span></div></div>
