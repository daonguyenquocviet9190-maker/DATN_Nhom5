...
import "../globals.css"; // Đã sửa đường dẫn lùi 2 cấp chính xác
...
export default function ClientLayout({ children }) {
  ...
  const [displayName, setDisplayName] = useState('Khách hàng'); // Quản lý tên hiển thị động
  ...
  // Tự động kiểm tra trạng thái và lấy tên người dùng từ localStorage khi tải trang
  useEffect(() => {
    ...
    if (savedStatus === 'true') {
      setIsLoggedIn(true);
      if (savedName) {
        setDisplayName(savedName); // Cập nhật tên vừa đăng ký/đăng nhập lên Header
      }
    }
  }, []);
  ...
  const handleLogout = () => {
    localStorage.removeItem('isLoggedIn'); // Xóa bộ nhớ đăng nhập
    // Giữ lại userDisplayName để nếu họ quay lại login vẫn nhận diện được, hoặc xóa luôn nếu muốn xóa sạch:
    // localStorage.removeItem('userDisplayName'); 
    setIsLoggedIn(false);
    setShowUserMenu(false);
    window.location.reload(); // Làm mới trình duyệt để đồng bộ lại giao diện sạch
  };

  // Tự động đóng dropdown tài khoản khi click ra ngoài vùng menu
  useEffect(() => {
  ...
  return (
    <html lang="vi">
      <body className="bg-white text-gray-900 font-sans antialiased">
                  ...
                  {!isLoggedIn ? (
                    ...
                    <div>
                      <div className="px-4 py-2 border-b border-gray-50 pb-3 mb-1">
                        ...
                        {/* Hiển thị tên động lấy từ localStorage */}
        ...
        {/* ================= NỘI DUNG THAY ĐỔI (ĐÃ SỬA LỖI CLASSNAME) ================= */}
